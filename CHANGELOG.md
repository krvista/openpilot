...
* Phase 43 (09-23): 43a 42a 의 도시 빠른 하강을 디바운스된 파지에만 적용(토크 160 Nm 팔은 35a 속도표로 복귀) — 얹은 손에서 권한이 펌핑되며 생긴 저속·도시 핸들 떨림 회귀 수정(5–60 km/h 떨림 창 9–12 % → 14 %, 빠른 하강 창에서 32 %); 43b 운전자가 가속 페달을 밟고 1.5→3.0 m/s² 가속할 때(≥20 km/h) 권한 천장 1.0→0.5 배 (급가속 중 파지 분당 16–37 회 vs 정속 2–11) (Kill: TQ_ARM_FLOOR_V=RATE_DN_FLOOR_V / ACCEL_YIELD_SCALE_V=[1,1])
* Phase 42 (09-20, 을지로 좌회전 파지 충돌): 42a 도시 속도 파지 시 권한 하강 바닥 0.03/프레임(35a 의 60 km/h 이상 한정을 전 속도로; 도시 파지 후 바닥 도달 p50 0.80 s → ~0.2 s), 42b 700 driver-Nm 이상 밀기는 0.1 s 안에 바닥, 42c 파지 플래그 깜빡임 뒤 0.3 s 핸들 앵커 유지(0.1 s 이상 놓으면 해제) (Kill: RATE_DN_FLOOR_V=[0,0.03] / SHOVE_RATE_DN=0 / ANCHOR_HOLD_FRAMES=0)
* Phase 41 (09-17): 안쪽 차선 확률 < 0.3 이고 손 뗀 상태(≥30 km/h, 깜빡이·차선변경 아님)에서 조향 요청 각속도 6 °/s 캡, 해제 시 1 s 램프 — 병합·분기에서 모델 재정렬을 따라가는 낚아채기(운전자 반대 토크 21건 중 14건이 이 대역)를 느린 드리프트로 (Kill: LOWCONF_CAP_DPS=0); 41-2 룩어헤드 캡 0.27→0.32 s (코너 진입 플랜→차량 0.7 s 중 요청 측 손잡이)
