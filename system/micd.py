...
class Mic:
  ...
  def micd_thread(self):
    ...
    # manager raised — either because `get_stream`'s @retry(attempts=7, delay=3)
    # gave up after ~21s, or because the active stream threw asynchronously
    # (e.g. PortAudio device disappearing on a USB/audio-HAL hiccup) — the
    # exception propagated out of `micd_thread` → `main()` → process exit with
    # code 1. The manager then reported `running=False shouldBeRunning=True`,
    # which selfdrived turned into EventName.processNotRunning (SOFT_DISABLE),
    # silently dropping cruise control mid-drive (drivelog evidence: 37
    # `processNotRunning` disengages across 5 routes on a Jeep GC).
    #
    # The microphone is non-safety-critical (only used for ambient noise → alert
    # volume), so a transient HAL error must NOT terminate the process. Instead
    # we keep the daemon alive and let the manager believe everything is fine;
    # if the device returns we transparently re-acquire the stream. We sleep
    # before retrying so we don't spin on a hard-broken device, and we log at
    # exception level so the failure is still visible in cloudlog.
    while True:
      try:
