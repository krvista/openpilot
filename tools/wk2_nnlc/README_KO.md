...
- **2026-09-19 홀드 하한 54→52km/h, LKAS 비트 52→50km/h (dbb1463)** — d67c5fe 실주행 검증(21라우트, 활성 5.6h): 63 미만 어시스트 12,121프레임, 홀드 창 EPS 응답 상관 0.82/이득 80%, 램프 밴드 [52,54) 상관 0.93/이득 100%, 비개입 63 컷 0건(32건 전부 54로 이동), 램프다운 0.50s. 폴트 3건은 모두 운전자 오버라이드 직후(1건은 64km/h 홀드 창 밖) → 저속 토크 아닌 오버라이드로 귀속. 폴트 중 램프다운 금지 추가. **[50,52) 램프 밴드는 아직 미검증** — 다음 주행 후 로 확인, 폴트 나오면 비트 하한 원복.
