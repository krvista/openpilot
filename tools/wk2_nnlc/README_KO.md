...
- **2026-09-19 홀드 하한 54→52km/h, LKAS 비트 52→50km/h (dbb1463)** — d67c5fe 실주행 검증(21라우트, 활성 5.6h): 63 미만 어시스트 12,121프레임, 홀드 창 EPS 응답 상관 0.82/이득 80%, 램프 밴드 [52,54) 상관 0.93/이득 100%, 비개입 63 컷 0건(32건 전부 54로 이동), 램프다운 0.50s. 폴트 3건은 모두 운전자 오버라이드 직후(1건은 64km/h 홀드 창 밖) → 저속 토크 아닌 오버라이드로 귀속. 폴트 중 램프다운 금지 추가. **[50,52) 램프 밴드는 아직 미검증** — 다음 주행 후 `hold_verify.py --hold-margin 3.0 --bit-margin 3.5`로 확인, 폴트 나오면 비트 하한 원복.
- **2026-09-19 차량용 범용 수정 6건 (b3f15ce)** — ① 학습값 백업: `LiveTorqueParameters`·`LiveParametersV2`·`CarParamsPrevRoute`에 BACKUP 플래그(sunnylink 백업/복원 대상; 캘리브레이션은 마운트 종속이라 제외) ② UI: NNLC/Enforce Torque "마지막에 켠 쪽 유지", Chrysler에선 둘 다 OFF 금지(PID 무음 회귀 차단) ③ torqued: |steeringTorque|>40(컬럼 단위) 프레임 학습 제외 ④ soundd: 연속 12회 실패 시 프로세스 종료→processNotRunning 가시 알림 ⑤ 개입 릴리스 후 0.4s 오차항 블렌드인 — v1·**v0**(NNLC ON일 때 실제 선택되는 컨트롤러)·NNLC 경로 모두 ⑥ 부팅 그레이스 중 "Steering assist initializing" 경고(custom 이벤트 @24). 검증: py_compile·capnp enum 로드·토글 상태기계/블렌드 램프 오프라인 시뮬. **미검증**: params_keys.h·custom.capnp는 장치 scons 빌드에서 컴파일(여기선 불가) — 첫 부팅 후 Settings›Software 커밋 확인 필수.
- **2026-09-20 52km/h 홀드 실주행 검증 PASS (b3f15ce, 3라우트 rlog, 활성 0.74h)** — 63 미만 어시스트 34,618프레임(활성의 13%), 홀드 창 EPS 응답 corr 0.89/이득 108%, 램프 밴드 [50,52) n=179 corr 0.99/이득 152%, 비개입 63 컷 0(8건 전부 52 하한), 램프다운 0.34s, **폴트 0**. → 홀드 −3.0/비트 −3.5 확정.
- **2026-09-20 조향 품질(b3f15ce)** — |dla−ala| 평균 0.070(v7 수렴 0.075 수준 회복, 신설치 0.132 대비), 떨림 23/min, 운전자 토크 p50 29(이전 96)·p90 150, 이벤트 0. torqued useParams=False(버킷 6,960, 시드 1.95/0.175 적용 중), liveDelay 0.29 estimated.
- **2026-09-20 우측 편향 측정** — 모델 차선선 기준 차 위치: OP 활성 평균 **−2.3cm(좌)**, 수동 운전 −5.7cm(좌); 라우트별 2d −6.5/−5.9, 2f −2.0/−8.1, 31 +1.0/+1.7. calib yaw +1.63°(8월과 동일), angleOffset −1.2~−1.8°(정비 후에도 유지 → 조향각센서 영점). 즉 모델 기준으로는 우편향 없음 → 체감 원인 후보: ① 운전자 선호(−6cm)보다 OP가 ~4cm 오른쪽 ② 카메라 마운트 횡오프셋(모델 기준 측정으로는 검출 불가). 조치: 5d10161 업데이트 후 `CameraOffset=+0.04`로 시작, lane_bias로 ACTIVE 평균이 −0.06 부근으로 이동하는지 확인(반대로 가면 부호 반전).
