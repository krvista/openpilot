...
from opendbc.car.scalar import interp as _interp, clip as _clip   # scalar, bit-identical to np.interp/np.clip (100 Hz path)
...
class CurvatureSteeringLimits:
  ...
  def apply_limits(self, apply_curvature: float, apply_curvature_last: float, v_ego: float, curvature: float,
    ...
    new_apply_curvature = float(_clip(apply_curvature, -max_curvature, max_curvature))

    # *** max lateral jerk limit ***
    max_jerk = (self.MAX_LATERAL_JERK / (v_ego ** 2)) * (steer_step * DT_CTRL)
    new_apply_curvature = float(_clip(new_apply_curvature, apply_curvature_last - max_jerk, apply_curvature_last + max_jerk))
    ...
    # prevent fault
    return float(_clip(new_apply_curvature, -self.CURVATURE_MAX, self.CURVATURE_MAX))
